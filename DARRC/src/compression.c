...
uint32_t compress_FASTx_files(char* filename_read_1, char* filename_read_2, bool pair_ended,
    ...
    printf("chiamo min_simReads_max_simkmers2\n");
...
struct filename_struct{
 ...
 struct input_data{
    int n_thread;
    Read_count*** read_count_array;
    Pvoid_t** reads_array;
    bool pair_ended;
    int size_minimizer;
    struct filename_struct filename_array[960/NUM_THREAD];
};

void* parallel_read (void* param){
    
    struct input_data* data = (struct input_data*)param;
    Pvoid_t reads = (Pvoid_t) NULL;
    PWord_t reads_value;
    Word_t pos_minimizer;
    Word_t pos_minimizer_tmp;
    Word_t  reads_flag;
    //estraggi i dati in input
    
    bool pair_ended=data->pair_ended;
    int size_minimizer=data->size_minimizer;
    int n_thread=data->n_thread;
    ...
    int nb_min_per_bin = CEIL((int)pow(4, size_minimizer), FOPEN_MAX_HARD - 64);
    Pvoid_t reads_array[960/NUM_THREAD][nb_min_per_bin];
    Read_count* read_count_array[960/NUM_THREAD][nb_min_per_bin];
    size_t size_buffer_read = SIZE_BUFFER;
    ...
    //variabile d'appoggio per il filenames
    struct filename_struct filename_temp;
    
for (it_bins = 0; it_bins < 960/NUM_THREAD; it_bins++){

        filename_temp = data->filename_array[it_bins];
        filenames=filename_temp.filename;
        ...
        for (it_buffer = 0; it_buffer < nb_min_per_bin; it_buffer++){
            ...
            if (filenames[it_buffer] != NULL){
                ...
                while (reads_value != NULL){
                    ...
                    *reads_value = (Word_t) list_reads;
                    ...
                    JLN(reads_value, reads, pos_minimizer_tmp);
                               ...
                               reads_array[it_bins][it_buffer]=reads;
                               read_count_array[it_bins][it_buffer]=read_count;
                ...
                JLFA(reads_flag, reads);
                   ...
                   data->reads_array=reads_array;
    data->read_count_array=read_count_array;
...
printf("\ncreate_super_reads\n");
    ...
    pthread_attr_t attr;
    struct input_data data[num_thread];
    struct input_data* result;
    void* retv;
    Pvoid_t reads = (Pvoid_t) NULL;
    PWord_t reads_value;
    Word_t  reads_flag;
    ...
    char* read;
    char** filenames;
    //variabili per lo scamnio di dati con i thread
    struct filename_struct filename_array[num_thread][(960)/NUM_THREAD];
...
for(int t=0;t<NUM_THREAD; t++){
    ...
    for (it_bins = 0; it_bins < (960)/NUM_THREAD; it_bins++){
        ...
        filenames = subdivide_bin(prefix_bin_name, it_bins+(t*(FOPEN_MAX_HARD - 64)/NUM_THREAD), size_minimizer, true);
        filename_array[t][it_bins].filename=filenames;

}
}
pthread_attr_init(&attr);

//creo i thread e lancio parallel_read
pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_JOINABLE);
    for(i = 0; i < num_thread; i++)
    {
    	data[i].pair_ended=pair_ended;
    	data[i].size_minimizer=size_minimizer;
	data[i].n_thread=i;
	
	 
    for (it_bins = 0; it_bins < (960)/NUM_THREAD; it_bins++){

	data[i].filename_array[it_bins]=filename_array[i][it_bins];
	}
	
		
        if( pthread_create(&th[i], &attr, parallel_read, (void*)&data[i]) )
        {
...
printf("\neseguo le join");
    ...
    printf("\n%d\n",j+1);
        ...
        result=(struct input_data*)retv;
        
        for (it_bins = 0; it_bins < (960)/NUM_THREAD; it_bins++){

        filenames=(char**)result->filename_array[it_bins].filename;
        ...
        for (it_buffer = 0; it_buffer < nb_min_per_bin; it_buffer++){
            ...
            if (filenames[it_buffer] != NULL){
                ...
                printf("debug_1\n");
                read_count=create_Read_count();
		read_count=result->read_count_array[it_bins][it_buffer];
		reads=result->reads_array[it_bins][it_buffer];
   		printf("debug_2\n");
        ...
        reassemble_bins_to_super_bins(prefix_bin_name, it_bins+(j*(960/num_thread)), filenames, size_minimizer);
...
{ printf("sono in min_simReads_max_simkmers2");
    ...
    printf("\nchiamo create_super_reads");
    create_super_reads(output_prefix, pair_ended, size_minimizer);
    printf("\nchiamato create_super_reads");
     printf("\nchiamo create_spanning_super_reads");
 ...
 printf("\nchiamato create_spanning_super_reads");
